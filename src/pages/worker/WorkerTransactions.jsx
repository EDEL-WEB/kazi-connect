...
export default function WorkerTransactions() {
  ...
  const statusColor = { completed: '#2e7d32', pending: '#e65100', failed: '#c62828' };

  return (
    <div style={s.page}>
      <div style={s.header}>
        <button style={s.back} onClick={() => navigate('/worker/wallet')}>← Wallet</button>
      ...
      {transactions.length > 0 && (
        <div style={s.tableWrap}>
          <table style={s.table}>
            <thead>
              <tr>{['Date', 'Type', 'Amount', 'Status'].map(h => <th key={h} style={s.th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {transactions.map((tx, i) => (
                <tr key={i} style={s.tr}>
                  ...
                  <td style={{ ...s.td, color: statusColor[tx.status] || '#555', fontWeight: 600, textTransform: 'capitalize' }}>{tx.status}</td>
                </tr>
              ))}
            </tbody>
          </table>
