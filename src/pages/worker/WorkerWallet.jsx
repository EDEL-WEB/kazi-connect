  ...
  return (
    <div style={s.page}>
      ...
      {wallet && (
        <>
          <div style={s.cards}>
            <div style={{ ...s.walletCard, background: 'linear-gradient(135deg,#00695c,#00897b)' }}>
              <div style={s.wLabel}>Available Balance</div>
              <div style={s.wAmount}>{money(wallet.balance)}</div>
            </div>
            <div style={{ ...s.walletCard, background: 'linear-gradient(135deg,#1565c0,#1976d2)' }}>
              <div style={s.wLabel}>Held in Escrow</div>
              <div style={s.wAmount}>{money(wallet.held_balance ?? 0)}</div>
            </div>
            <div style={{ ...s.walletCard, background: 'linear-gradient(135deg,#4a148c,#7b1fa2)' }}>
              <div style={s.wLabel}>Total Earned</div>
              <div style={s.wAmount}>{money(wallet.total_earned ?? 0)}</div>
            </div>
          </div>
          <button style={s.txBtn} onClick={() => navigate('/worker/transactions')}>View Transactions →</button>
        </>
      )}
    </div>
  );
