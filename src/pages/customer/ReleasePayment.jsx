  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.card}>
        ...
        <h2 style={s.title}>Release Payment</h2>
        <p style={s.sub}>Confirm that the job has been completed to your satisfaction. The funds held in escrow will be released to the worker.</p>
        {error && <div style={s.error}>{error}</div>}
        <div style={s.btns}>
          <button style={s.btnGhost} onClick={() => navigate(`/jobs/${jobId}`)}>Cancel</button>
          <button style={s.btnGreen} onClick={handleRelease} disabled={loading}>{loading ? 'Releasing...' : 'Release Payment'}</button>
        </div>
      </div>
    </div>
  );
